                    ...
                    <div className="text-xs text-muted-foreground">
                      ...
                      {reg.quantity > 1 && (
                        <span className="font-medium text-foreground ml-1">
                          (Total: ${reg.ticket.price * reg.quantity})
                        </span>
                      )}
                    </div>
                  </div>
      ...
      {myTickets.length > 0 && (
        <div className="mt-6 pt-4 border-t">
          <div className="flex flex-col xs:flex-row xs:items-center xs:justify-between gap-2 text-sm text-muted-foreground">
            <span>
              {myTickets.length} ticket{myTickets.length !== 1 ? 's' : ''} total
            </span>
            <span className="font-medium text-primary">
              Total spent: ${myTickets.reduce((acc, reg) => acc + (reg.ticket.price * reg.quantity), 0)}
            </span>
          </div>
        </div>
      )}
