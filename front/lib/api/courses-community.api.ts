 ...
 */
function transformCourse(backendCourse: any): any {
  ...
  const sections = (backendCourse.sections || []).map((section: any) => ({
    ...
    chapters: (section.chapitres || []).map((chapter: any) => ({
      ...
      duration: chapter.duree || 0,
      order: chapter.ordre || chapter.order || 0,
      isFree: chapter.isPreview !== false || !chapter.isPaidChapter,
      price: chapter.prix || chapter.price || 0,
