  ...
  useEffect(() => {
    const loadAnalytics = async () => {
      ...
      try {
        ...
        const [overviewRes, membersRes, engagementRes] = await Promise.all([
          api.creatorAnalytics.getOverview({ from, to }).catch((e:any) => { if (e?.statusCode===402||e?.statusCode===403) setAnalyticsGated(true); return null }),
        ...
        const engagementSeries = (engagementRes?.data?.series) || (engagementRes?.series) || []
        setEngagementData(Array.isArray(engagementSeries) ? engagementSeries : [])
