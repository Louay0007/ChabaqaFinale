...
export function CourseCard({ course }: CourseCardProps) {
  ...
  const chaptersTotal: number = Array.isArray(course.sections)
    ? course.sections.reduce((acc: number, s: any) => acc + (Array.isArray(s.chapters) ? s.chapters.length : 0), 0)
    : (course.chaptersCount ?? 0)

  const priceNumber: number = typeof course.price === 'number'
    ? course.price
    : (course.priceType === 'free' ? 0 : Number(course.price ?? 0))
  ...
  const resolvedCourse = {
    ...
    price: priceNumber,
    sections: Array.isArray(course.sections) ? course.sections : Array.from({ length: sectionsCount }, () => ({ chapters: [] })),
  ...
  function getCoursePricing(course: any) {
    if (course.price === 0) {
      const flatChapters = (Array.isArray(course.sections) ? course.sections : []).flatMap((s: any) => Array.isArray(s.chapters) ? s.chapters : [])
      const paidChapters = flatChapters.filter((c: any) => c?.price && c.price > 0)
