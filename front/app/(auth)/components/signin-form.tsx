...
import { loginAction, verifyTwoFactorAction } from "../signin/actions"
...
export default function SignInForm({ onSuccess }: SignInFormProps = {}) {
  ...
  const handleInitialSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // First, logout to clear any previous sessions
      console.log('AUTH - Logging out before login attempt');
      try {
        ...
        console.log('Logout before login failed:', err);
        // Continue anyway
      }
      ...
      if (json?.requires2FA) {
        ...
        console.log('AUTH - Login successful, redirecting by role');
        if (onSuccess) {
          onSuccess()
        }
        await redirectAfterAuth()
      } else {
        setError(json?.message || "Une erreur s'est produite")
      }
    } catch (error) {
  ...
  const handleTwoFactorSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result = await verifyTwoFactorAction({ email, verificationCode })
  ...
  const handleGoogleLogin = () => {
    ...
    const backendBase = apiBase.endsWith("/api") ? apiBase.slice(0, -4) : apiBase
    // We cannot compute email-based slug before OAuth returns; send to /profile and let profile page redirect to slug.
